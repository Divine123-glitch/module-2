...
def add_slide(prs, title, bullets, layout=1): # layout=1 is Title and Content
	...
	for bullet in bullets:
		p = tf.add_paragraph()
		p.text = bullet
		p.level = bullet.count('.') # Simple indent for sub-bullets (optional)
		p.alignment = PP_ALIGN.LEFT
		for run in p.runs:
			run.font.size = Pt(18)

	# Add footer (simple text box at bottom)
	left = Inches(0.5)
	top = Inches(6.5)
	width = Inches(9)
	height = Inches(0.5)
	...
	tf_footer.text = "United to Prevent Violence Against Women and Girls | www.cewhin.com"
	tf_footer.paragraphs[0].alignment = PP_ALIGN.CENTER
	for run in tf_footer.paragraphs[0].runs:
		run.font.size = Pt(12)
...
print("PPTX file generated: Sales_Report.pptx")
